...
class DaemonFailsafeTests(unittest.TestCase):
    def test_unit_has_execstoppost(self) -> None:
        ...
        self.assertIn("After=graphical-session.target", sdk)
        self.assertIn("WantedBy=graphical-session.target", sdk)
        self.assertIn("Environment=DISPLAY=:0", sdk)
        self.assertIn("Environment=XAUTHORITY=%h/.Xauthority", sdk)
        self.assertNotIn("WantedBy=default.target", sdk)
        self.assertNotIn("User=root", sdk)
        self.assertNotIn("set_pwm", sdk)
        gui = (ROOT / "packaging/chromaflow-gui.service").read_text(encoding="utf-8")
        self.assertIn("WantedBy=graphical-session.target", gui)
        self.assertNotIn("WantedBy=default.target", gui)
        enable = (ROOT / "packaging/enable-session.sh").read_text(encoding="utf-8")
        self.assertIn("disable --now chromaflow-sdk.service", enable)
        self.assertIn("enable --now chromaflow-gui.service", enable)
        spawn = (ROOT / "crates/chromaflow-core/src/openrgb_spawn.rs").read_text(encoding="utf-8")
        self.assertIn("restart_sdk_once", spawn)
        self.assertIn("sdk_opt_in", spawn)
        self.assertIn("DISPLAY", spawn)
        self.assertIn("try_wait", spawn)
        self.assertNotIn("mem::forget", spawn)
        self.assertNotIn("chromaflowd", spawn)
        self.assertNotIn("set_pwm", spawn)
        nvidia = (ROOT / "crates/chromaflow-core/src/nvidia_fan_apply.rs").read_text(encoding="utf-8")
        self.assertIn("last_same", nvidia)
        self.assertNotIn("set_pwm", nvidia)
        fans = (ROOT / "crates/chromaflow-core/src/nvidia_fans.rs").read_text(encoding="utf-8")
        self.assertIn("from_secs(2)", fans)
        boot = (ROOT / "crates/chromaflow-core/src/openrgb_boot.rs").read_text(encoding="utf-8")
        self.assertIn("restart_sdk_once", boot)
        self.assertIn("/tmp/.X11-unix/X0", boot)
        self.assertNotIn("chromaflowd", boot)
        app = (ROOT / "apps/desktop/src/App.svelte").read_text(encoding="utf-8")
        self.assertIn("researchDevices(lightInv)", app)
